...
namespace Cyan
{
    void RenderTarget::attachColorBuffer(Texture* _texture)
    {
        glBindFramebuffer(GL_FRAMEBUFFER, m_frameBuffer);
        switch (_texture->m_type)
        {
            case Texture::Type::TEX_2D:
                glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + m_numColorBuffers, GL_TEXTURE_2D, _texture->m_id, 0);
                m_numColorBuffers += 1;
                break;
            case Texture::Type::TEX_CUBEMAP:
                for (u32 f = 0; f < 6u; f++)
                {
                    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + m_numColorBuffers, GL_TEXTURE_CUBE_MAP_POSITIVE_X + f, _texture->m_id, 0);
                    m_numColorBuffers += 1;
                }
                break;
            default:
