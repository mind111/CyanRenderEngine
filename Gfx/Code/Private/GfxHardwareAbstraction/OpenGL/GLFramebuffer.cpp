...
namespace Cyan
{
    ...
    void GLFramebuffer::bindColorBuffer(GHTextureCube* colorBufferCube, u32 faceIndex, u32 bindingUnit, u32 mipLevel)
    {
        assert(bindingUnit < FRAMEBUFFER_MAX_NUM_COLOR_BUFFERS);
        auto glTexture = dynamic_cast<GLTextureCube*>(colorBufferCube);
        assert(glTexture != nullptr);
        i32 resolution = 0;
        glTexture->getMipSize(resolution, mipLevel);
        assert(resolution == m_width && resolution == m_height);
        glNamedFramebufferTexture2DEXT(m_name, GL_COLOR_ATTACHMENT0 + bindingUnit, GL_TEXTURE_CUBE_MAP_POSITIVE_X + faceIndex, glTexture->getName(), mipLevel);
        m_colorBuffers[bindingUnit] = colorBufferCube;
    }
