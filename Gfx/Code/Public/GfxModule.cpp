...
namespace Cyan
{
    ...
    static void processRendererUI()
    {
        auto renderer = SceneRenderer::get();

        ImGui::Begin("Renderer Settings");
        static i32 currentRenderMode = 0;
        const i32 itemCount = (i32)RenderMode::kCount;
        const char* renderModeNames[itemCount] = { 
            "ResolvedSceneColor",
            "SceneAlbedo",
            "SceneNormal",
            "SceneDepth",
            "SceneMetallicRoughness",
            "SkyIrradiance",
            "DirectLighting",
        ...
        if (ImGui::Combo("##Render Mode:", &currentRenderMode, renderModeNames, itemCount))
        {
            renderer->m_settings.renderMode = (RenderMode)currentRenderMode;
        }

        if (ImGui::CollapsingHeader("ReSTIR SSDO", ImGuiTreeNodeFlags_DefaultOpen))
        {
            IMGUI_CHECKBOX("Apply SSAO", renderer->m_settings.ReSTIRSSDOSettings.bApplySSAO);
            IMGUI_CHECKBOX("Spatial Pass", renderer->m_settings.ReSTIRSSDOSettings.bEnableSpatialPass);
            IMGUI_SLIDER_FLOAT("Kernel Radius", &renderer->m_settings.ReSTIRSSDOSettings.spatialPassKernelRadius, 0.f, 1.f);
            IMGUI_SLIDER_INT("Sample Count", &renderer->m_settings.ReSTIRSSDOSettings.spatialPassSampleCount, 0, 32);
        }
        if (ImGui::CollapsingHeader("ReSTIR SSGI", ImGuiTreeNodeFlags_DefaultOpen))
        {
            IMGUI_CHECKBOX("Enable", renderer->m_settings.ReSTIRSSGISettings.bEnable);
            IMGUI_SLIDER_FLOAT("Indirect Irradiance Boost", &renderer->m_settings.ReSTIRSSGISettings.indirectIrradianceBoost, 1.f, 16.f);
            IMGUI_CHECKBOX("SSGI Spatial Pass", renderer->m_settings.ReSTIRSSGISettings.bEnableSpatialPass);
            IMGUI_SLIDER_FLOAT("SSGI Spatial Pass Kernel Radius", &renderer->m_settings.ReSTIRSSGISettings.spatialPassKernelRadius, 0.f, 1.f);
            IMGUI_SLIDER_INT("SSGI Spatial Pass Sample Count", &renderer->m_settings.ReSTIRSSGISettings.spatialPassSampleCount, 0, 32);
        }
        if (ImGui::CollapsingHeader("SSGI Denoiser", ImGuiTreeNodeFlags_DefaultOpen))
        {
            IMGUI_CHECKBOX("Enable Denoising", renderer->m_settings.ReSTIRSSGISettings.bEnableDenoising);
            IMGUI_SLIDER_INT("Num Denoising Passes", &renderer->m_settings.ReSTIRSSGISettings.numDenoisingPasses, 1, 7);
            IMGUI_CHECKBOX("Apply Normal Edge Stopping:", renderer->m_settings.ReSTIRSSGISettings.bNormalEdgeStopping);
            IMGUI_SLIDER_FLOAT("Normal Sigma", &renderer->m_settings.ReSTIRSSGISettings.sigmaN, 0.f, 16.f);
            IMGUI_CHECKBOX("Apply Position Edge Stopping", renderer->m_settings.ReSTIRSSGISettings.bPositionEdgeStopping);
            IMGUI_SLIDER_FLOAT("Position Sigma", &renderer->m_settings.ReSTIRSSGISettings.sigmaP, 0.f, 32.f);
        }
        ImGui::End();
    }
    ...
    void GfxModule::renderUI(Frame& frame)
    {
        ...
        while (!frame.mainThreadUICommands.empty())
        {
            const auto& command = frame.mainThreadUICommands.front();
            command(s_imguiCtx);
            frame.mainThreadUICommands.pop();
        }

        // render SceneRenderer UI
        processRendererUI();
