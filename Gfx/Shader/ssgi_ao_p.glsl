...
void main()
{
    ...
    float xz = texture(sceneDepthBuffer, pixelCoord).r;
    vec3 n = texture(sceneNormalBuffer, pixelCoord).xyz * 2.f - 1.f;
	...
	for (int j = 0; j < numSamplesPerDirection; ++j)
	{
		...
		// calculate theta1 
		float sz1 = texture(sceneDepthBuffer, sampleCoord).r;
		...
		float sz2 = texture(sceneDepthBuffer, sampleCoord).r;
