...
uniform sampler2D u_aoTemporalOutput;
...
void main()
{
	...
	float deviceDepth = texture(u_sceneDepthTex, texCoord).r;
	...
	vec2 texelSize = 1.f / textureSize(u_aoTemporalOutput, 0).xy;
...
#define NUM_TAPS 7
	...
	for (int i = 0; i < NUM_TAPS; ++i)
	{
		for (int j = 0; j < NUM_TAPS; ++j)
		{
			...
			{
				float aoTap = texture(u_aoTemporalOutput, sampleCoord).r;
				float depthTap = texture(u_sceneDepthTex, sampleCoord).r;
