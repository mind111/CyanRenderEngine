...
{
    Reservoir r;
    r.z.radiance = texture(u_reservoirRadiance, sampleCoord).rgb;
    r.z.position = texture(u_reservoirPosition, sampleCoord).rgb;
    r.z.normal = texture(u_reservoirNormal, sampleCoord).rgb * 2.f - 1.f;
    vec3 wSumMW = texture(u_reservoirWSumMW, sampleCoord).rgb;
    r.wSum = wSumMW.x;
    r.M = wSumMW.y;
    r.W = wSumMW.z;
    return r;
