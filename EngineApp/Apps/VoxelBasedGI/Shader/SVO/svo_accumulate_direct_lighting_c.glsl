...
layout (r32f) uniform image3D u_voxelDirectLightingR;
layout (r32f) uniform image3D u_voxelDirectLightingG;
layout (r32f) uniform image3D u_voxelDirectLightingB;
...
void main()
{
	...
	if (node.bLeaf > 0 && node.bSubdivide > 0)
	{
		...
		imageAtomicAdd(u_voxelDirectLightingR, voxelCoord, fragment.directLighting.r);
		imageAtomicAdd(u_voxelDirectLightingG, voxelCoord, fragment.directLighting.g);
		imageAtomicAdd(u_voxelDirectLightingB, voxelCoord, fragment.directLighting.b);
	}
}
