...
uniform sampler2D u_ssDiffuseHitMask;
uniform sampler2D u_ssDiffuseRayInfo;
...
void main()
{
	...
	// indirect lighting
	vec3 ro = worldSpacePosition + n * 0.01f;

	// diffuse
	{
		vec3 diffuseRadiance = vec3(0.f);

		vec4 rayDirectionAndDistance = texture(u_ssDiffuseRayInfo, psIn.texCoord0).rgba;
		float diffuseHitMask = texture(u_ssDiffuseHitMask, psIn.texCoord0).r;

		vec3 rd = rayDirectionAndDistance.rgb;
		float ndotl = max(dot(n, rd), 0.f);
		if (diffuseHitMask > 0.f)
		{
			// sample screen space radiance for indirect lighting 
		}
		else
		{
			// continue tracing in voxel space
			HitRecord hitRecord = hierarchicalRayMarchingVoxelGrid(ro + rd * rayDirectionAndDistance.w, rd);
			if (!hitRecord.bHit)
			{
