...
out float outHitMask;
out vec4 outRayDirectionAndMarchedDistance;
...
void main()
{
	...
	HitRecord screenSpaceHitRecord;
	hizTrace(ro, rd, screenSpaceHitRecord);

	outHitMask = 0.f;
	outRayDirectionAndMarchedDistance = vec4(rd, screenSpaceHitRecord.continueT);

	if (screenSpaceHitRecord.result == HitResult_Hit)
	{
		outHitMask = 1.f;
	}
}
