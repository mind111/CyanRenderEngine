...
uniform sampler2D u_prevFrameAO;
...
float calcAO(vec3 worldSpacePosition, vec3 worldSpaceNormal)
{
	vec3 worldSpaceRo = worldSpacePosition + worldSpaceNormal * 0.001f; 
	vec3 localSpaceRo = (inverse(u_heightFieldTransformMatrix) * vec4(worldSpaceRo, 1.f)).xyz;

	float outAO = 0.f;
	const float numSamples = 1;
	vec3 ro = localSpaceRo;
	for (int i = 0; i < numSamples; ++i)
	{
		vec3 worldSpaceRd = uniformSampleHemisphere(worldSpaceNormal);
		vec3 localSpaceRd = (inverse(u_heightFieldTransformMatrix) * vec4(worldSpaceRd, 0.f)).xyz;

		HitRecord hitRecord = hierarchicalRayMarching(ro, localSpaceRd);
		if (!hitRecord.bHit) 
		{
			outAO += 1.f;
		}
...
void main()
{
    ...
    {
		vec3 worldSpacePosition = transformPositionScreenToWorld(vec3(uv, sceneDepth) * 2.f - 1.f);
		vec3 pp = vec3(uv, sceneDepth) * 2.f - 1.f;
		vec3 n = normalize(texture(u_sceneNormal, uv).rgb * 2.f - 1.f);
		vec3 ao = vec3(calcAO(worldSpacePosition, n));
		...
		if (u_frameCount > 0)
		{
				...
				&& reprojectedClipSpacePos.y >= 0.f && reprojectedClipSpacePos.y <= 1.f)
			{
				...
				if (depthDiff < 0.15f)
				{
					...
					float prevFrameAO = texture(u_prevFrameAO, reprojectedUV).r;
					ao = vec3(prevFrameAO * .9f  + ao.r * .1f);
				}
			}
		}
