...
layout (location = 0) out vec3 outTemporalReservoirDirection;
layout (location = 1) out vec4 outTemporalReservoirWSumMWT;

uniform int u_frameCount;
uniform sampler2D u_sceneDepth;
uniform sampler2D u_sceneNormal;
uniform sampler2D u_prevFrameSceneDepth;
uniform vec2 u_renderResolution;

uniform mat4 u_prevFrameViewMatrix;
uniform mat4 u_prevFrameProjectionMatrix;
uniform mat4 u_viewMatrix;
uniform mat4 u_projectionMatrix;

uniform sampler2D u_temporalReservoirDirection;
uniform sampler2D u_temporalReservoirWSumMWT;
...
void updateReservoir(inout Reservoir r, in vec3 sampleDirection, in float sampleWeight, in float targetPdf)
{
	r.wSum += sampleWeight;
...
void main()
{
	...
	temporalReservoir.direction = n;
	temporalReservoir.wSum = 0.f;
	temporalReservoir.M = 0.f;
	temporalReservoir.W = 0.f;
	temporalReservoir.targetPdf = 0.f;

	if (u_frameCount > 0)
	{
		vec4 reprojectedClipSpacePos = u_prevFrameProjectionMatrix * u_prevFrameViewMatrix * vec4(worldSpacePosition, 1.f);
		reprojectedClipSpacePos /= reprojectedClipSpacePos.w;
		reprojectedClipSpacePos = reprojectedClipSpacePos * .5f + .5f;

		// lookup temporal cache
		if (reprojectedClipSpacePos.x >= 0.f && reprojectedClipSpacePos.x <= 1.f 
			&& reprojectedClipSpacePos.y >= 0.f && reprojectedClipSpacePos.y <= 1.f)
		{
			vec3 reprojectedViewSpacePos = (u_viewMatrix * vec4(worldSpacePosition, 1.f)).xyz;
			float prevFrameDepth = texture(u_prevFrameSceneDepth, reprojectedClipSpacePos.xy).r;
			vec3 prevFrameWorldSpacePos = transformPositionScreenToWorld(vec3(reprojectedClipSpacePos.xy, prevFrameDepth) * 2.f - 1.f);
			vec3 prevFrameViewSpacePos = (u_prevFrameViewMatrix * vec4(prevFrameWorldSpacePos, 1.f)).xyz;

			float depthDiff = abs(reprojectedViewSpacePos.z - prevFrameViewSpacePos.z) / abs(prevFrameViewSpacePos.z);
			if (depthDiff < 0.15f)
			{
				vec2 reprojectedUV = reprojectedClipSpacePos.xy;
				temporalReservoir.direction = texture(u_temporalReservoirDirection, reprojectedUV).rgb;

				vec4 wSumMW = texture(u_temporalReservoirWSumMWT, reprojectedUV).rgba;
				temporalReservoir.wSum = wSumMW.r;
				temporalReservoir.M = int(wSumMW.g);
				temporalReservoir.W = wSumMW.b;
				temporalReservoir.targetPdf = wSumMW.a;
			}
		}
	}
	...
	vec3 worldSpaceRo = worldSpacePosition + n * 0.001f;
	...
	vec3 sampleDirection = n;
	float visibility = 0.f;
	if (!hitRecord.bHit)
	{
		// visibility = max(dot(n, rd), 0.f); // target pdf
		visibility = 1.f; // target pdf
		sampleDirection = rd;
	}

	// float srcPdf = 1.f / (2.f * PI);
	float srcPdf = 1.f;
	float sampleWeight = visibility / srcPdf;
	updateReservoir(temporalReservoir, sampleDirection, sampleWeight, visibility);
	
	// clamp M
	const float kMaxReservoirM = 32.f;
	if (temporalReservoir.M > kMaxReservoirM)
	{
		float avgWeight = temporalReservoir.wSum / temporalReservoir.M;
		float clampedWSum = avgWeight * kMaxReservoirM;

		temporalReservoir.wSum = clampedWSum;
		temporalReservoir.M = kMaxReservoirM;
		temporalReservoir.W = temporalReservoir.wSum / max((temporalReservoir.M * temporalReservoir.targetPdf), 1e-4);
	}

	outTemporalReservoirDirection = temporalReservoir.direction;
	outTemporalReservoirWSumMWT = vec4(temporalReservoir.wSum, temporalReservoir.M, temporalReservoir.W, temporalReservoir.targetPdf); 
}
