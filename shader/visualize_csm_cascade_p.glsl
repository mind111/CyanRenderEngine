...
void main() 
{
	...
	if (depth <= 0.999999f) 
	{
		...
		vec4 csmPrimaryViewClip = csmPrimaryViewProjectionMatrix * vec4(csmPrimaryViewSpacePosition, 1.f);
		csmPrimaryViewClip /= csmPrimaryViewClip.w;
		vec3 csmPrimaryViewScreenCoord = csmPrimaryViewClip.xyz * .5f + .5f;
		if (csmPrimaryViewSpacePosition.z <= 0.f && csmPrimaryViewScreenCoord.x >= 0.f && csmPrimaryViewScreenCoord.x <= 1.f && csmPrimaryViewScreenCoord.y >= 0.f && csmPrimaryViewScreenCoord.y <= 1.f)
		{
			...
			vec3 tint = mix(green, red, t);
			outColor = mix(texture(sceneColor, psIn.texCoord0).rgb, tint, .9);
		}
	}
}
