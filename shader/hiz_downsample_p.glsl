...
void main()
{
	vec2 inputTextureSize = textureSize(sceneDepthBuffer, 0).xy;
	vec2 inputTexelSize = 1.f / inputTextureSize;
	vec2 outputTexelSize = 1.f / outputResolution;
	// A --- B
	// |     |
	// C --- D
	...
	for (int i = 0; i < numSamplesInY; ++i)
	{
		for (int j = 0; j < numSamplesInX; ++j)
		{
			...
			{
				minDepth = min(texture(sceneDepthBuffer, sampleCoord).r, minDepth);
			}
		}
	}
