...
namespace Cyan
{
    static void calcPerspectiveViewFrustum(const Camera& camera, f32 n, f32 f, glm::vec3& na, glm::vec3& nb, glm::vec3& nc, glm::vec3& nd, glm::vec3& fa, glm::vec3& fb, glm::vec3& fc, glm::vec3& fd)
    {
        if (camera.m_projectionMode == Camera::ProjectionMode::kPerspective)
        {
    ...
    void CascadedShadowMap::render(Scene* scene, DirectionalLight* light, const Camera& camera)
    {
        if (camera.m_projectionMode == Camera::ProjectionMode::kPerspective)
        {
            ...
            {
                m_cascades[i].n = camera.m_perspective.n + cascadeBoundries[i] * (camera.m_perspective.f - camera.m_perspective.n);
                m_cascades[i].f = camera.m_perspective.n + cascadeBoundries[i + 1] * (camera.m_perspective.f - camera.m_perspective.n);

                // setup light view camera
                Camera& camera = m_cascades[i].lightCamera;
                camera.setProjectionMode(Camera::ProjectionMode::kOrthographic);

                // todo: worldUp needs to be careful of being in the same direction as camera's worldSpaceUp
                glm::vec3 worldUp = Camera::worldUpVector();
            ...
            for (u32 i = 0; i < kNumCascades; ++i)
            {
                ...
                const Camera& camera = m_cascades[i].lightCamera;
                SceneCamera::ViewParameters viewParameters;
                viewParameters.renderResolution = m_resolution;
                viewParameters.aspectRatio = (f32)m_resolution.x / m_resolution.y;
                viewParameters.cameraForward = camera.worldSpaceForward();
                viewParameters.cameraRight = camera.worldSpaceRight();
                viewParameters.cameraUp = camera.worldSpaceUp();
                viewParameters.cameraPosition = camera.m_worldSpacePosition;
                viewParameters.viewMatrix = camera.viewMatrix();
                viewParameters.projectionMatrix = camera.projectionMatrix();

                renderer->renderSceneDepth(m_cascades[i].depthTexture.get(), scene, viewParameters);
            }

            m_lastPrimaryViewCamera = camera;
        }
        else
        {
    ...
    void CascadedShadowMap::setShaderParameters(ProgramPipeline* p)
    {
