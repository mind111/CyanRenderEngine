...
namespace Cyan
{
    ...
    void Engine::submitForRendering()
    {
        ...
        frame.mainThreadUICommands = std::move(s_frameUICommands);
    ...
    void Engine::addSceneRenderingTask(const SceneRenderingStage& sceneRenderingStage, const char* taskName, std::function<void(SceneView& view)>&& taskLambda)
    {
        std::string addTaskName = "AddSceneRenderingPass" + std::string(taskName);

        enqueueFrameGfxTask(
            RenderingStage::kPreSceneRendering,
            addTaskName.c_str(),
            [sceneRenderingStage, task = std::move(taskLambda)](Frame& frame) mutable {
                auto view = (*frame.views)[0];
                view->addTask(sceneRenderingStage, std::move(task));
            }
        );
    }
}
