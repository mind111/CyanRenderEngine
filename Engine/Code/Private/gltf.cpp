    ...
    namespace gltf
    {
        ...
        void Gltf::importMaterials()
        {
            for (i32 i = 0; i < m_materials.size(); ++i)
            {
                ...
                // create and setup the material instance
                ...
                if (baseColorTextureIndex >= 0)
                {
                    ...
                    if (texture != nullptr)
                    {
                        mi->setTexture("mp_albedoMap", texture);
                        mi->setFloat("mp_hasAlbedoMap", 0.5f);
                    }
                }
                ...
                if (normalTextureIndex >= 0)
                {
                    ...
                    if (texture != nullptr)
                    {
                        mi->setTexture("mp_normalMap", texture);
                        mi->setFloat("mp_hasNormalMap", .5f);
                    }
                }

                f32 metallicFactor = gltfMatl.pbrMetallicRoughness.metallicFactor;
                f32 roughnessFactor = gltfMatl.pbrMetallicRoughness.roughnessFactor;
                ...
                if (metallicRoughnessIndex >= 0)
                {
                    ...
                    if (texture != nullptr)
                    {
                        mi->setTexture("mp_metallicRoughnessMap", texture);
                        mi->setFloat("mp_hasMetallicRoughnessMap", .5f);
                    }
                }
            }
        }
        ...
        static void from_json(const json& m_jsonObject, Node& node)
        {
            ...
            json children;
            if (jsonFind(children, m_jsonObject, "children"))
            {
                children.get_to(node.children);
            }
        }
